...
contract ClearingHouse {
    ...
    function newLoan(
        ...
        uint256 callFee,
        uint256 callPeriod,
        bytes memory borrowerSignedMessage,
        bytes memory lenderSignedMessage
    ) public {
        // encode the loan terms
        bytes memory loanTerms = abi.encode(borrower, lender, borrowToken, collateralToken, borrowAmount, collateralAmount, interestRate, callFee, callPeriod);
        
        // use ecrecover to check that the borrower signed the loan terms
        require(ecrecover(loanTerms, 27, borrowerSignedMessage[0:32], borrowerSignedMessage[32:64]) == borrower, "borrower signature invalid");

        // use ecrecover to check that the lender signed the loan terms
        require(ecrecover(loanTerms, 27, lenderSignedMessage[0:32], lenderSignedMessage[32:64]) == lender, "lender signature invalid");
