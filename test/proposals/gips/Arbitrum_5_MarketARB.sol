...
contract Arbitrum_5_MarketARB is GovernorProposal {
    function name() public view virtual returns (string memory) {
        return "Arbitrum_5_MarketARB";
    }

    constructor() {
        require(
            block.chainid == 42161,
            "Arbitrum_5_MarketARB: wrong chain id"
        );
    }
    ...
    string internal constant PEG_TOKEN = "ARB";
    uint256 internal constant MARKET_ID = 4; // gauge type / market ID
    ...
    function run(address/* deployer*/) public virtual {
        ...
        _addStep(
            ...
            string.concat(
                "Approve ARB on PSM for initial mintAndEnterRebase() [market ",
                Strings.toString(MARKET_ID),
                "]"
            )
        );
        _addStep(
            ...
            string.concat(
                "Initial PSM.mintAndEnterRebase() with 15 ARB [market ",
                Strings.toString(MARKET_ID),
                "]"
            )
        );
